...
export default {
  ...
  mounted() {
    ...
    this.$axios.post("/employee/get/detail/" + this.emp_id).then((response) => {
      ...
      this.$set(this.formdata, "fathername", response.data["fathername"]);
      let not_too_check = [
        "post",
        "company",
        "appointment",
      ...
      Object.keys(this.formdata).forEach((item) => {
        console.log("--", item);
        if (item == "dob") {
          console.log("-----", new Date(payload[item]));

          this.$set(this.formdata, item, new Date(payload[item]));
          // this.$set(this.formdata, item, response.data[item]);
        }
        if (item == "dateeff") {
          this.$set(this.formdata, item, new Date(payload[item]));
          // this.$set(this.formdata, item, response.data[item]);
        ...
        if (item == "benefits") {
          this.$set(this.formdata, item, payload[item]);
          // this.$set(this.formdata, item, response.data[item]);
        }
        ...
        if (
          ...
          item != "dateeff" &&
          item != "dob" &&
          !not_too_check.includes(item)
        ) {
          console.log("-----~~~~", item, payload[item]);
          this.$set(this.formdata, item, payload[item]);
        }

        if (not_too_check.includes(item)) {
          console.log("-----AAAA~~~~", item, payload[item]);
          if (payload[item].length != 0) {
            console.log("-----~~~~", item, payload[item][0].id);
            this.$set(this.formdata, item, payload[item][0].id);
          } else {
            console.log("-----~~~~NONE", item, payload[item]);
  ...
  methods: {
    ...
    submitData(e) {
      if (this.checkData(e)) {
        ...
        let self = this;
        this.$set(this.formdata, "emp_id", this.emp_id);
